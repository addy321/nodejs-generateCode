...
import pojo.Baserole;

public class BaseroleDao extends daoTemplate implements pojoDao<Baserole>{
    
    public BaseroleDao(Connection conn) {
        super(conn);
    ...
    @Override
    public int save(Baserole data) throws Exception {
        String sql="insert into Base_Role(Id,CreateTime,CreatorId,Deleted,RoleName) values(?,?,?,?,?)";
    ...
    @Override
    public int update(Baserole data) throws Exception {
        StringBuffer sql=new StringBuffer("update Base_Role set ");
        ...
        if(data.getRolename()!=null && !"".equals(data.getName())){
            sql.append("RoleName=?,");
            values.add(data.getRolename());
        }
    ...
    @Override
    public List<Baserole> query(String condition, List<Object> values, Integer page, Integer pageSize) throws Exception {
        String sql="select Id,CreateTime,CreatorId,Deleted,RoleName from Base_Role";
        ...
        List<Baserole> datas=new ArrayList<Baserole>();
        while(rs.next()){
            Baserole data=new Baserole();
            ...
            data.setRoleName(rs.getRolename("RoleName"));	
            datas.add(data);
        }
        return datas;
    ...
    public long count(String condition, List<Object> values) throws Exception {
        String sql="select count(1) from Base_Role";
        if(condition!=null&&!"".equals(condition)&&values!=null&&values.size()>0){
            sql=sql+" where "+condition;
        }
